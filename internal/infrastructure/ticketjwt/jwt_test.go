...
var (
	...
	userRepo   = mockUserRepository{
		users: []domain.User{{ID: uuid.New(), FirstName: "firstname", LastName: "lastname"}},
	}
)
...
func TestNewTokenSuccess(t *testing.T) {
	p, err := ticketjwt.NewJwtAuthProvider(mockGetUserSuccessFunc, publicKey, privateKey, ticketjwt.RS512, 1000)
	assert.NoError(t, err, "NewJwtAuthProvider should not error")
	_, err = p.NewToken(domain.User{ID: uuid.New()})
...
func TestValidateTokenSuccess(t *testing.T) {
	p, err := ticketjwt.NewJwtAuthProvider(mockGetUserSuccessFunc, publicKey, privateKey, ticketjwt.RS512, 1000)
	assert.NoError(t, err, "NewJwtAuthProvider should not error")
	token, err := p.NewToken(domain.User{ID: uuid.New()})
	assert.NoError(t, err, "successful userSubjectFunc should not error")
...
func TestGetTokenUserSuccess(t *testing.T) {
	...
	userRepo.users = append(userRepo.users, expectUser)

	p, err := ticketjwt.NewJwtAuthProvider(mockGetUserSuccessFunc, publicKey, privateKey, ticketjwt.RS512, 1000)
	assert.NoError(t, err, "NewJwtAuthProvider should not error")
	token, err := p.NewToken(domain.User{ID: testUUID})
	assert.NoError(t, err, "valid user should not error")

	gotUser, err := p.GetUser(context.Background(), token)
	assert.NoError(t, err)
	assert.Equal(t, expectUser, gotUser)
}
...
func TestUserDeleted(t *testing.T) {
	testUUID := uuid.New()
	userRepo.users = append(userRepo.users, domain.User{
		ID:        testUUID,
...
func (m *mockUserRepository) GetUser(ctx context.Context, userID uuid.UUID) (domain.User, error) {
	idx := slices.IndexFunc(m.users, func(u domain.User) bool {
		return u.ID == userID
	})
	if idx == -1 {
...
func mockGetUserSuccessFunc(ctx context.Context, userID uuid.UUID) (user domain.User, err error) {
	u, _ := userRepo.GetUser(ctx, userID)
	return u, nil
}

func mockGetUserErrFunc(ctx context.Context, userID uuid.UUID) (user domain.User, err error) {
